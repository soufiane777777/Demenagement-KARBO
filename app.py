...
# Le chemin vers ton fichier JSON (le fichier que tu viens de créer)
SERVICE_ACCOUNT_FILE = 'credentials.json'

# Scopes (permissions demandées)
SCOPES = ['https://www.googleapis.com/auth/spreadsheets']

# Création des credentials à partir du fichier
creds = Credentials.from_service_account_file(SERVICE_ACCOUNT_FILE, scopes=SCOPES)

# Construction du service Sheets
service = build('sheets', 'v4', credentials=creds)

# ID de ta Google Sheet (extrait de l'URL)
SPREADSHEET_ID = '1YbilKuAOpaL2As8iQ-pUrj_Oo11gVu6lBdmh-IvzDGw'  # remplace par l'ID réel

# Plage à lire (exemple : feuille1!A1:D10)
RANGE_NAME = 'Feuille1!A1:D10'

# Appel de l'API pour récupérer les valeurs
...
# Chemin vers ton fichier JSON de credentials
SERVICE_ACCOUNT_FILE = 'C:\bureau\python course\PythonProject\app demenagement'

# Portée pour Google Sheets
SCOPES = ['https://www.googleapis.com/auth/spreadsheets']

# ID de ta Google Sheet (visible dans l'URL)
SPREADSHEET_ID = 'YbilKuAOpaL2As8iQ-pUrj_Oo11gVu6lBdmh-IvzDGw'
...
def export_to_sheets(dataframe, sheet_range='A1'):
    sheet = init_sheets_api()
    # Convertis dataframe en liste de listes
    ...
    sheet.values().update(spreadsheetId=SPREADSHEET_ID,
                          ...
                          body=body).execute()
    st.success("Données exportées vers Google Sheets !")
